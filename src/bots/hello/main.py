...
from hl_core.hl_client import (
    make_clients,
)  # function: Info/Exchange の初期化（安全ガード込み）
...
def main() -> None:
    ...
    print(
        f"[hello] network={settings.network} dry_run={settings.dry_run} address={_short(address)}"
    )
    ...
    if address:
        try:
            state = info.user_state(
                address
            )  # function: SDKの例にある基本API（README準拠）
        ...
        print(
            "[hello] ユーザ状態の取得をスキップしました。`.env` に HL_ACCOUNT_ADDRESS を設定すると表示できます。"
        )

    # function: Exchange は作成済みでも、このサンプルでは一切注文しない
    if exchange is None:
        print(
            "[hello] exchange=None（鍵未設定 or 署名無効）。発注機能は作られていません。"
        )
    else:
